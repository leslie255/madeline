...
fn gen_inside_fn(
    ...
    for instruction in body {
        match instruction {
            IRInstruction::Add(_, _) => todo!(),
            IRInstruction::Sub(_, _) => todo!(),
            IRInstruction::Mul(_, _) => todo!(),
            IRInstruction::Div(_, _) => todo!(),
            IRInstruction::Alloc(_) => todo!(),
            ...
            IRInstruction::Store { id: vreg_id, rhs } => {
                ...
                target.push(Instruction::Mov(
                    Operand::WordPtr(
                        X86WordSize::from(dtype),
                        EvalTreeNode::Sub(
                            Box::new(EvalTreeNode::Reg(X64Register::Rbp)),
                            Box::new(EvalTreeNode::Num(stack_location as u64)),
                        ),
                    ),
                    rhs_operand,
                ));
            ...
            IRInstruction::Ret(ret_val) => {
                if let Some(ret_val) = ret_val {
                    let (operand_type, operand) = gen_operand(*ret_val, &stack_alloc, &vreg_allocations);
                    let operand_type = operand_type;
                    let operand_wordsize = X86WordSize::from(operand_type);
                    target.push(Instruction::Mov(
                        Operand::Reg(X64Register::Rax.of_size(operand_wordsize)),
                        operand,
                    ));
            ...
            illegal => panic!("{:?} is illegal as root", illegal),
        }
    }
}
...
fn gen_operand(
    ...
    match instruction {
        ...
        IRInstruction::Load { id, dtype } => (dtype,Operand::WordPtr(
            X86WordSize::from(dtype),
            EvalTreeNode::Sub(
                Box::new(EvalTreeNode::Reg(X64Register::Rbp)),
                Box::new(EvalTreeNode::Num(
